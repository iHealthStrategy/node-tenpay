...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox, openLog, logger} = {}, debug = false) {
    ...
    this.urls = sandbox ? {
      ...
      get_wxpayface_authinfo: 'https://payapp.weixin.qq.com/face/get_wxpayface_authinfo',
  ...
  async _request(params, type, cert = false) {
    ...
    keys.forEach(function (key) {
      if (params[key] != "" && params[key] != undefined) {  //如果参数的值为空不参与签名；
        sortedParams[key] = params[key]  //参数名区分大小写；
      }

    })
    // 安全签名
    sortedParams.sign = this._getSign(sortedParams, sortedParams.sign_type);
    // 创建请求参数
    let pkg = {method: 'POST', dataType: 'text', data: util.buildXML(sortedParams)};
    if (cert) {
      pkg.pfx = this.pfx;
    ...
    // this.log('post data =>\r\n%s\r\n', pkg.data);
    this.logger.info('[tenpay]', `${type} req:`, JSON.stringify(pkg.data));
    let {status, data} = await urllib.request(this.urls[type], pkg);
    if (status !== 200) throw new Error('request fail');
    this.logger.info('[tenpay]', `${type} res:`, JSON.stringify(data));
  ...
  get_wxpayface_authinfo(params) {
    ...
    return this._request(pkg, 'get_wxpayface_authinfo');
  }
